...
export class PrismaTaskRepository implements TaskRepository {
  async findById(id: string): Promise<Task | null> {
    ...
    if (!p) return null;
    return new Task(p.id, p.title, p.description ?? undefined, p.done, p.ownerId ?? undefined, p.createdAt);
  }
  async list(ownerId?: string): Promise<Task[]> {
    ...
    return ps.map((p) => new Task(p.id, p.title, p.description ?? undefined, p.done, p.ownerId ?? undefined, p.createdAt));
  }
